...
mod test {
    ...
    #[test]
    fn hashtags2() {
        let uws = "#hashtag#hashtag2 #hash_tag";
        let result = vec![
            PositionalToken {
                source: uws,
                offset: 0,
            ...
            PositionalToken {
                ...
                offset: 18,
                length: 9,
                token: Token::Struct(Struct::Hashtag("hash_tag".to_string())),
            },
        ];
        let lib_res = uws
            .into_tokenizer(TokenizerParams::v1().add_option(TokenizerOptions::StructTokens))
            .collect::<Vec<_>>();
        check_results(&result, &lib_res, uws);
    }
    ...
    fn mention2() {
        ...
        let result = vec![
            PositionalToken {
                ...
                offset: 0,
                length: 8,
                token: Token::Struct(Struct::Mention("hashtag".to_string())),
            },
            PositionalToken {
                source: uws,
                offset: 8,
                length: 9,
                token: Token::Struct(Struct::Mention("hashtag2".to_string())),
            },
            PositionalToken {
                source: uws,
                offset: 17,
            ...
            PositionalToken {
                ...
                offset: 18,
                length: 9,
                token: Token::Struct(Struct::Mention("hash_tag".to_string())),
            },
        ];
        let lib_res = uws
            .into_tokenizer(TokenizerParams::v1().add_option(TokenizerOptions::StructTokens))
    ...
    fn numerical_default() {
        ...
        let result = vec![
            ...
            PositionalToken {
                ...
                offset: 3,
                length: 2,
                token: Token::Word(Word::Number(Number::ZeroInteger {
                    i: 2,
                    s: "02".to_string(),
                })),
            },
    ...
    fn get_lang_test(lng: Lang) -> (String, Vec<PositionalToken>) {
        let uws = match lng {
            ...
            Lang::Kor => {
                "플레이스테이션 은 소니 컴퓨터 엔터테인먼트가 개발한 세 번째 가정용 게임기이다. 마이크로소프트의 엑스박스 360, 닌텐도의 Wii와 경쟁하고 있다. 이전 제품에서 온라인 플레이 기능을 비디오 게임 개발사에 전적으로 의존하던 것과 달리 통합 온라인 게임 서비스인 플레이스테이션 네트워크 서비스를 발매와 함께 시작해 제공하고 있으며, 탄탄한 멀티미디어 재생 기능, 플레이스테이션 포터블과의 연결, 고화질 광학 디스크 포맷인 블루레이 디스크 재생 기능 등의 기능을 갖추고 있다. 2006년 11월 11일에 일본에서 처음으로 출시했으며, 11월 17일에는 북미 지역, 2007년 3월 23일에는 유럽과 오세아니아 지역에서, 대한민국의 경우 6월 5일부터 일주일간 예약판매를 실시해, 매일 준비한 수량이 동이 나는 등 많은 관심을 받았으며 6월 16일에 정식 출시 행사를 열었다"
            }
            Lang::Jpn => {
